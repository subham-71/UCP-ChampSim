...
void greedy_repartition(){
	...
	uint32_t winner = 0;
	vector<float> uNext(NUM_CPUS);

	while (balance){
		for (uint32_t i=0; i<NUM_CPUS; i++){
			uNext[i] = get_util(i, allocations[i], allocations[i]+1);
			if (uNext[i]>uNext[winner]){
                winner = i;
            }
		}
		allocations[winner]++;
	...
	for (int i = 0; i<NUM_CPUS; i++){
        newPartition[i] = allocations[i];
    }
    totalAccess = 0;
    for (auto &x: umonglobal)
        x = 0;
}
...
float get_max_mu(int i, int alloc, int balance){
    ...
    for (int j=1; j<=balance; j++){
        float mu = get_util(i, alloc, alloc+j);
        // max_mu = max(max_mu, get_util(i, alloc, alloc+j));
        if (mu>max_mu){
            max_mu = mu;
        }
    }
...
void lookahead_repartition(){
    uint32_t balance = LLC_WAY - NUM_CPUS;
	vector<uint32_t> allocations(NUM_CPUS, 1);
    vector<float> max_mu(NUM_CPUS);
    vector<uint32_t> blocks_req(NUM_CPUS);
    while (balance){
        for(uint32_t i=0; i<NUM_CPUS; i++){
            max_mu[i] = get_max_mu(i, allocations[i], balance);
            blocks_req[i] = get_blocks_req(i, allocations[i], balance);
        }
        uint32_t winner = 0;
        for(uint32_t i=0; i<NUM_CPUS; i++){
            if(max_mu[winner]<max_mu[i]){
                winner=i;
            }
        }
